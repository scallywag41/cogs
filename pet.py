...
###
    ...
    async def comehere(self, ctx):
        ...
        """
        msg = ctx.message
        await self.check_voice(msg.author, msg)
        return
    ...
    async def bark(self, ctx):
        ...
        if self.downloader["DOWNLOADING"]:
            await self.bot.say("I'm already barking.")
            return
        msg = ctx.message
        ...
        if localplaylists and ("data/audio/localtracks/" not in name and "\\" not in name):
            if name in localplaylists:
                ...
                if await self.is_alone_or_admin(msg):
                    ...
                    await self.bot.say("I'm in bark mode. Controls are disabled if you're in a room with multiple people.")
            else:
