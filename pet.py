...
class Pet:
    ...
    def __init__(self, bot):
        self.bot = bot
        self.pet = fileIO("data/pet/pet.json", "load")
        self.settings = fileIO("data/pet/settings.json", "load")
        self.payday_register = {}

    @commands.group(name="pet", pass_context=True)
    async def _pet(self, ctx):
        """Pet operations"""
        if ctx.invoked_subcommand is None:
            await send_cmd_help(ctx)

    @_pet.command(pass_context=True, no_pm=True)
    async def register(self, ctx):
        """Adopts a pet from the shelter"""
        user = ctx.message.author
        if user.id not in self.pet:
            self.pet[user.id] = {"name" : user.name, "balance" : 100}
            fileIO("data/pet/pet.json", "save", self.pet)
            await self.bot.say("{} Account opened. Current balance: {}".format(user.mention, str(self.check_balance(user.id))))
        else:
            await self.bot.say("{} You already a pet.".format(user.mention))

    @_pet.command(pass_context=True)
    async def balance(self, ctx, user : discord.Member=None):
        ...
        if not user:
            ...
            if self.account_check(user.id):
                ...
                await self.bot.say("{} You don't have a pet. Type !register to adopt one.".format(user.mention, str(self.check_balance(user.id))))
        else:
            if self.account_check(user.id):
                balance = self.check_balance(user.id)
                await self.bot.say("{}'s balance is {}".format(user.name, str(balance)))
            else:
                await self.bot.say("That user has no pet account.")
    ...
    async def transfer(self, ctx, user : discord.Member, sum : int):
        ...
        if author == user:
            ...
            return
        if sum < 1:
            await self.bot.say("You need to transfer at least 1 soul.")
            return
        if self.account_check(user.id):
            if self.enough_money(author.id, sum):
                self.withdraw_money(author.id, sum)
                self.add_money(user.id, sum)
                logger.info("{}({}) transferred {} souls to {}({})".format(author.name, author.id, str(sum), user.name, user.id))
                await self.bot.say("{} souls have been transferred to {}'s account.".format(str(sum), user.name))
            else:
                await self.bot.say("You don't have that pet.")
        else:
            await self.bot.say("That user has no pet.")

    @_pet.command(name="set", pass_context=True)
    @checks.admin_or_permissions(manage_server=True)
    async def _set(self, ctx, user : discord.Member, sum : int):
        """Sets money of user's pet 

        Admin/owner restricted."""
        author = ctx.message.author
        done = self.set_money(user.id, sum)
        if done:
            logger.info("{}({}) set {} souls to {} ({})".format(author.name, author.id, str(sum), user.name, user.id))
            await self.bot.say("{}'s souls have been set to {}".format(user.name, str(sum)))
        else:
            await self.bot.say("User has no pet.")
        ...
        if self.account_check(id):
            if id in self.payday_register:
                ...
                if seconds  >= self.settings["PAYDAY_TIME"]: 
                    ...
                    await self.bot.say("{} Here, take some souls. Enjoy! (+{} souls!)".format(author.mention, str(self.settings["PAYDAY_CREDITS"])))
                else:
            ...
            await self.bot.say("{} You need an account to receive souls.".format(author.mention))
    ...
    async def leaderboard(self, top : int=10):
        ...
        if top < 1:
            top = 10
        pet_sorted = sorted(self.pet.items(), key=lambda x: x[1]["balance"], reverse=True)
        if len(pet_sorted) < top:
            top = len(pet_sorted)
        topten = pet_sorted[:top]
        highscore = ""
        place = 1
        for id in topten:
        ...
        if highscore:
            ...
            await self.bot.say("There are no pets.")

    @commands.command(pass_context=True)
    async def comehereboy(self, ctx):
        """Tells pet to come."""
        await self.bot.join_voice_channel(ctx.message.author.voice_channel)
        #v_channel = ctx.message.author.voice_channel
        #await self.bot.say(v_channel)

    @commands.command(pass_context=True)
    async def gohere(self, ctx, channel : discord.Channel):
        if channel.type==discord.ChannelType.voice:
            await self.bot.join_voice_channel(channel)

    @commands.command(pass_context=True)
    async def goaway(self):
    ...
    async def slot_machine(self, message, bid):
        ...
        if line[0] == ":two:" and line[1] == ":two:" and line[2] == ":six:":
            ...
            await self.bot.send_message(message.channel, "Souls left: {}".format(str(self.check_balance(message.author.id))))
            return True
        self.add_money(message.author.id, bid)
        await self.bot.send_message(message.channel, "Current souls: {}".format(str(self.check_balance(message.author.id))))

    @commands.group(pass_context=True, no_pm=True)
    @checks.admin_or_permissions(manage_server=True)
    async def petset(self, ctx):
        """Changes pet module settings"""
        if ctx.invoked_subcommand is None:
            msg = ""
            for k, v in self.settings.items():
                msg += str(k) + ": " + str(v) + "\n"
            msg += "\nType help petset to see the list of commands."
    ...
    async def slotmin(self, bid : int):
        ...
        await self.bot.say("Minimum bid is now " + str(bid) + " souls.")
    ...
    async def slotmax(self, bid : int):
        ...
        await self.bot.say("Maximum bid is now " + str(bid) + " souls.")
        fileIO("data/economy/settings.json", "save", self.settings)
    ...
    async def paydaycredits(self, credits : int):
        """Souls earned each payday"""
        self.settings["PAYDAY_CREDITS"] = credits
        await self.bot.say("Every payday will now give " + str(credits) + " souls.")
        fileIO("data/economy/settings.json", "save", self.settings)
    ...
    def check_balance(self, id):
        if self.account_check(id):
            return self.pet[id]["balance"]
        else:
            return False

    def add_money(self, id, amount):
        if self.account_check(id):
            self.pet[id]["balance"] = self.pet[id]["balance"] + int(amount)
            fileIO("data/pet/pet.json", "save", self.pet)
        else:
            return False
    ...
    def withdraw_money(self, id, amount):
        if self.account_check(id):
            if self.pet[id]["balance"] >= int(amount):
                self.pet[id]["balance"] = self.pet[id]["balance"] - int(amount)
                fileIO("data/pet/pet.json", "save", self.pet)
            else:
                return False
